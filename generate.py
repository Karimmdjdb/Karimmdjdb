...
def get_date() :
    today = date.today()
    birthday = date(today.year, 3, 7)
    if(birthday < today) :
        birthday = date(today.year + 1, 3, 7)
    remains = (birthday - today).days
    today = today.strftime("%A %d %B %Y")
    remains = "today is my birthday ! 🎉" if remains == 0 else f"({remains} days until my birthday)."
    return (today, remains)
...
- 📆 Today is: **{get_date()[0]}** {get_date()[1]}
